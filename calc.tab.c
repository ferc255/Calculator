...
#line 111 "calc.tab.c" /* yacc.c:339  */
...
union YYSTYPE
{
#line 42 "calc.y" /* yacc.c:355  */
	
	value_t number;

#line 161 "calc.tab.c" /* yacc.c:355  */
};
...
#line 176 "calc.tab.c" /* yacc.c:358  */
...
#line 56 "calc.y" /* yacc.c:1646  */
    {
				...
				{
			 		case VT_INT:
						printf("%lld\n", (yyvsp[-1].number).llint);
						break;
			 		case VT_FLOAT:
						printf("%Lf\n", (yyvsp[-1].number).ldl);
						break;
					case VT_COMPLEX:
						if (cimagl((yyvsp[-1].number).xldl) >= 0.0)
						{
							printf("%Lf + %Lfi\n", creall((yyvsp[-1].number).xldl), cimagl((yyvsp[-1].number).xldl));
						}
						else
						{
							printf("%Lf - %Lfi\n", creall((yyvsp[-1].number).xldl), -cimagl((yyvsp[-1].number).xldl));
...
#line 85 "calc.y" /* yacc.c:1646  */
    {
			 value_type_t max_type = max((yyvsp[-2].number).type, (yyvsp[0].number).type);

			 (yyval.number).type = max_type;
			 cast_value(&(yyvsp[-2].number), max_type);
			 cast_value(&(yyvsp[0].number), max_type);

			 switch (max_type)
			 {
			 	case VT_INT:
					 (yyval.number).llint = (yyvsp[-2].number).llint + (yyvsp[0].number).llint;
					 break;
			 	case VT_FLOAT:
					 (yyval.number).ldl = (yyvsp[-2].number).ldl + (yyvsp[0].number).ldl;
					 break;
			 	case VT_COMPLEX:
					(yyval.number).xldl = (yyvsp[-2].number).xldl + (yyvsp[0].number).xldl;
					break;
			 }
         }
...
#line 151 "calc.y" /* yacc.c:1646  */
    {
			 ...
			 switch (max_type)
			 {
			 	...
			 	case VT_FLOAT:
					...
					break;
			 	case VT_COMPLEX:
					if (creall((yyvsp[0].number).xldl) != 0.0 || cimagl((yyvsp[0].number).xldl) != 0.0)
					{
						(yyval.number).xldl = (yyvsp[-2].number).xldl / (yyvsp[0].number).xldl;
					}
					else
